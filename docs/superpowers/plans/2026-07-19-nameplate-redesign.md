...
- **색은 토큰으로만 표현한다. 토큰 밖 하드코딩 색상값 금지.**

  디자인 팔레트 5개 — 화면의 성격을 결정하는 색:
  `--cast:#23262A` `--iron:#565D63` `--machined:#C9CDD0` `--plate:#ECEDEB` `--oxide:#B23A1E`

  기능 토큰 4개 — 특정 용도에만 쓰는 값. 아래 명시된 용도 외 사용 금지:
  `--white` 누끼 이미지 프레임 배경 (갤러리 컷이 흰 배경이므로 프레임도 흰색이어야 이미지가 떠 보이지 않는다) 및 `--oxide` 위 텍스트
  `--veil` 라이트박스 오버레이
...
- **CSS 선택자 중첩 깊이 2 이하.** 단, **상태 클래스(`.open`, `.active`)가 부모에 붙는 구조**에서는 깊이 3까지 허용한다 (기존 JS가 부모에 클래스를 부여하므로 불가피). 그 외의 깊이 3 이상은 금지.
  섹션 간격은 단일 유틸리티로 통일, 개별 재정의 금지.
...
:root {
  ...
  --white:          #FFFFFF;                 /* 누끼 이미지 프레임 배경 전용 */
  --veil:           rgba(35, 38, 42, 0.94);  /* 라이트박스 오버레이 전용 */
  --hairline-dark:  rgba(255, 255, 255, 0.12); /* 어두운 면 위의 구분선 */
  --edge-light:     rgba(255, 255, 255, 0.65); /* 명판 가장자리 인셋 하이라이트 */
...
/* 견적요청 — oxide 허용처 1/3 */
.btn--quote { background: var(--oxide); border-color: var(--oxide); color: var(--white); }
.btn--quote:hover { background: var(--cast); border-color: var(--cast); color: var(--white); }
...
.nav__cta {
  ...
  background: var(--oxide); color: var(--white);      /* oxide 허용처 1/3 */
...
.mobile-contact-btn {
  ...
  padding: 15px; border: 0; background: var(--oxide); color: var(--white);
...
.site-footer .data-table td { border-bottom-color: var(--hairline-dark); }
...
.footer-base {
  ...
  margin-top: 40px; padding-top: 20px; border-top: 1px solid var(--hairline-dark);
...
.prod-plate img { border-block: 1px solid var(--machined); background: var(--white);
...
.prod-hero-img { background: var(--white); padding: 32px; aspect-ratio: 1; object-fit: contain; }

.spec-figure { margin-top: 20px; }
.spec-figure img { cursor: zoom-in; background: var(--white); }
...
.lightbox { display: none; position: fixed; inset: 0; z-index: 2000;
            background: var(--veil); place-items: center; padding: 40px; }
...
.cert-figure img { background: var(--white); padding: 20px; aspect-ratio: 3/4; object-fit: contain; }
```
...
.cf-input, .cf-textarea, .cf-select {
  ...
  background: var(--white); border: 1px solid var(--machined);
...
.gallery-item { background: var(--white); border: 1px solid var(--machined); padding: 16px;
